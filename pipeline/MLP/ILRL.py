...
def main():
    ...
    for epoch in range(num_epochs):
        ...
        for _ in range(batch_size):
            ...
            # Incorporate reward into the loss
            reward_tensor = torch.tensor(reward, dtype=torch.float32).to(device)
            total_loss = imitation_loss - reward_tensor  # Subtract reward to maximize it
        ...
        print(f"Epoch {epoch + 1} completed with average loss: {average_epoch_loss}, imitation loss: {epoch_imitation_loss / batch_size}, reward: {epoch_reward / batch_size}")
...
#Plot the loss after each epoch
    ...
    model_save_path = os.path.join(os.path.dirname(__file__), "ILRL_SIMPLE.pth")
