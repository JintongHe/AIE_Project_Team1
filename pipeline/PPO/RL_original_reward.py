...
def ppo_train(policy, value_function, env, agent, state_dim, action_dim, num_epochs=200, steps_per_epoch=4000,
    ...
    for t in range(num_epochs * steps_per_epoch):
        ...
        next_state, target_velocity_reward, done, _ = env.step(body_action)
        ...
        imitation_reward = torch.exp(-F.mse_loss(torch.tensor(expert_action).to(device), torch.tensor(action).to(device)))
        
        reward = target_velocity_reward + survival_reward + imitation_reward

        # vr = target_velocity_reward/reward

        # vs = survival_reward/reward
        # vi = imitation_reward/reward
        # print("target velocity reward ration", vr)
        # print("survival reward ratio", vs)
        # print("imitation reward ratio", vi)
...
def main():
    ...
    torch.save(best_policy_state, 'new_22_states_survival_pros.pth')
