    ...
    for t in range(num_epochs * steps_per_epoch):
        ...
        if epoch_ended:
            ...
            if len(episode_rewards) > 0:
                ...
                if mean_reward > (max_ep_len-50):
                    num_good += 1
                    if num_good > 4:
                        num_good = 0
                        max_ep_len += 100
                        print(f"Max episode length increased to {max_ep_len}")
                # Save best policy
...
def main():
    ...
    best_policy_state, best_reward = ppo_train(
        ...
        num_epochs=6000,           # Increased for BipedalWalker
    ...
    torch.save(best_policy_state, 'running_16.pth')
