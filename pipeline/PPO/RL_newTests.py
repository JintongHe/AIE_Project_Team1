...
def ppo_train(policy, value_function, env, agent, state_dim, action_dim, num_epochs=200, steps_per_epoch=4000,
              ...
              device="mps", initial_noise=0.01, final_noise=0.1):
    """
    ...
    for t in range(num_epochs * steps_per_epoch):
        # Calculate current noise level - linearly increase from initial to final
        current_progress = t / total_steps
        current_noise_level = initial_noise + (final_noise - initial_noise) * current_progress
        ...
        with torch.no_grad():
            ...
            noise = torch.randn_like(ankle_state_tensor) * current_noise_level
            noisy_ankle_state = ankle_state_tensor + noise
            action, log_prob = policy.sample_action(noisy_ankle_state)
            value = value_function(noisy_ankle_state)
        ...
        # Calculate reward with survival bonus and torque penalty
        survival_bonus = 1.0  # Base survival reward
        torque_penalty = 0.5 * abs(action.item())  # Penalty proportional to torque magnitude

        if done:
            reward = -100  # Large negative reward for falling
        else:
            reward = survival_bonus - torque_penalty  # Survival bonus minus torque penalty
...
def main():
    ...
    best_policy_state, best_reward = ppo_train(
        ...
        device=device,
        initial_noise=0.005,       # Initial noise level for exploration
        final_noise=0.03           # Final noise level for exploration
    )

    # Save the best policy state
    torch.save(best_policy_state, 'new_36_states_survivalener_norm_noise_pros.pth')
